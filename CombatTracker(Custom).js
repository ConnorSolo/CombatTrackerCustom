 ...
 * Edited by ConnorSolo for use in personal game
*/
...
//            let prevSM = token.get('statusmarkers');
    ...
    rollInitiative = (selected, sort) => {
        ...
        selected.forEach(s => {
            ...
            //Connor Edit Starts; I commented out the line below because I replaced it with a different check down below.
            //whisper = (token.get('layer') === 'gmlayer') ? 'gm ' : '',
            //Connor Edit Ends
            let bonus = parseFloat(getAttrByName(token.get('represents'), state[state_name].config.initiative_attribute_name, 'current')) || 0;
            let roll = (config.turnorder.ini_die) ? randomInteger(config.turnorder.ini_die) : 0;
            //pr = (Math.round(pr) !== pr) ? pr.toFixed(2) : pr;
			//Connor Edit Starts; Added rollstyle, initmod, and npcflag variables to check 5e OGL sheet. I also added a ternary operator to check if the token is an npc and if so to change whisper to be to the GM.
			let rollstyle = String(getAttrByName(token.get('represents'), 'initiative_style', 'current'));
			let initmod = String(getAttrByName(token.get('represents'), 'initmod', 'current'));
			let npcflag = String(getAttrByName(token.get('represents'), 'npc', 'current'));
			let whisper = (npcflag === '1') ? 'gm ' : '';
			//Connor Edit Ends
                
			//Connor Edit Starts; Added if statement to allow for rolling with advantage and altered chat message to show both rolls and lowerd the font size to 14.
            if (rollstyle == '{@{d20},@{d20}}kh1') {
				let advroll = randomInteger(20);
                let highroll = Math.max(roll, advroll);
                let lowroll = Math.min(roll, advroll);
                let pr = highroll + bonus;
                pr = (Math.round(pr) !== pr) ? pr.toFixed(2) : pr;
                ...
                if(state[state_name].config.turnorder.show_initiative_roll){
                    ...
                    <div style="text-align: center"> \
                        <b style="font-size: 14pt;"> \
                            <span style="border: 1px solid green; padding-bottom: 2px; padding-top: 4px;">[['+highroll+'+'+bonus+']]</span><br><br> \
                        </b> \
                    </div> \
                    <div style="text-align: center"> \
                        <b style="font-size: 10pt;"> \
                            <span style="border: 1px solid red; padding-bottom: 2px; padding-top: 4px;">[['+lowroll+'+'+bonus+']]</span><br><br> \
                        </b> \
                    </div>'
                    //Connor Edit; Added the code in the line below to put an image of the token in the chat message
                    makeAndSendMenu(contents, '<img src="'+token.get('imgsrc')+'" style="height: 2.5em;float:left;margin-right:2px;"> ' + token.get('name') + ' Initiative', whisper);
                }
                
                //addToTurnorder({ id: token.get('id'), pr: highroll+bonus, custom: '', pageid: token.get('pageid') });
                addToTurnorder({ id: token.get('id'), pr, custom: '', pageid: token.get('pageid') });
            }
            //Connor Edit Ends
			
			//Connor Edit Starts; Added if statement to allow for rolling with an initiative mod and altered the chat message to show the extra bonus and lowered the font size to 14pt.
            else if (initmod == '1d4') {
                let intuitiondie = randomInteger(4);
                let pr = roll + bonus + intuitiondie;
                pr = (Math.round(pr) !== pr) ? pr.toFixed(2) : pr;
                ...
                if(state[state_name].config.turnorder.show_initiative_roll){
                    ...
                    <div style="text-align: center"> \
                        <b style="font-size: 14pt;"> \
                            <span style="border: 1px solid green; padding-bottom: 2px; padding-top: 4px;">[['+roll+'+'+bonus+'+'+intuitiondie+']]</span><br><br> \
                        </b> \
                    </div>'
                    //Connor Edit; Added the code in the line below to put an image of the token in the chat message
                    makeAndSendMenu(contents, '<img src="'+token.get('imgsrc')+'" style="height: 2.5em;float:left;margin-right:2px;"> ' + token.get('name') + ' Initiative', whisper);
                ...
                //addToTurnorder({ id: token.get('id'), pr: roll+bonus+intuitiondie, custom: '', pageid: token.get('pageid') });
                addToTurnorder({ id: token.get('id'), pr, custom: '', pageid: token.get('pageid') });
            }
            //Connor Edit Ends
			
			//Connor Edit Starts; I added an else statement becoause the enclosed code became redundant with the above if statements and would mess up the rolls.
			//I also added the if statement that checks if the token is an npc and if it is then its roll is whispered to the GM
			else {
				let pr = roll + bonus;
                pr = (Math.round(pr) !== pr) ? pr.toFixed(2) : pr;
			...
			//Connor Edit Starts; Commented out this section because my above edits make it redundant
			/*if(state[state_name].config.turnorder.show_initiative_roll){
                let contents = ' \
   ...
   getConditionString = (token) => {
    ...
    NextRound = () => {
        ...
        if(state[state_name].config.turnorder.reroll_ini_round){
            ...
            sortTurnorder();
            //Connor Edit Starts; Sends chat API commands to pause then unpause TrackerJacker in order to announce the first character's turn
            sendChat(script_name, "!tj -pause");
            sendChat(script_name, "!tj -start");
            //Connor Edit Ends
        }else{
            NextTurn();
        }
...
*/
    ...
    checkStatusInfo = () => {
        if(typeof StatusInfo === 'undefined'){
			//Connor Edit Starts; I commented out this reminder because it was annoying
        ...
        const defaults = {
            config: {
                ...
                turnorder: {
                    ...
                    ini_die: 20,
					//Connor Edit Starts; Changed show_initiative_roll and reroll_ini_round to true as default
                    show_initiative_roll: true,
                    auto_sort: true, 
                    reroll_ini_round: true,
					//Connor Edit Ends
                    skip_custom: true,
                },
                timer: {
